...
readSciMatFromMM f = do
  contents <- readFile f
  let rows = map (map S.fromString) (map words (lines contents))
      mat  = V.fromList $ map V.fromList rows
  return mat
...
main = do
  ...
  let outDir = parent outFile
  doesDirectoryExist outDir 
    >>= (\e -> case e of
          True -> return ()
          False -> do
  ...
  map (addComment outFile) comments
